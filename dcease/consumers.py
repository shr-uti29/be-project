...
onto = get_ontology(
    "D:/Sem 7/project/ontology versions/latestlatestlatest.owl")
    ...
    def connect(self):
        ...
        for i in sym:
            if i not in c:
                ...
                c[i] = c[i]+1
        WSConsumer.countdic = (
            dict(sorted(c.items(), key=lambda item: item[1], reverse=1)))
    ...
    def receive(self, text_data):
        ...
        if text_data == "yes":
            ...
            if n == WSConsumer.countdic[list(WSConsumer.countdic.keys())[0]]:
                ...
                l = list(set(l))
                self.send(json.dumps({'message': "Disease Name:", 'message1': list(WSConsumer.countdic.keys())[
                          0], 'message2': p, 'message3': l, 'message4': "Want to move forward?"}))
                ...
                if WSConsumer.flag == 1:
                    WSConsumer.k = WSConsumer.countdic[list(
                        WSConsumer.countdic.keys())[0]]
                    WSConsumer.flag = 0
                if WSConsumer.k == WSConsumer.countdic[list(WSConsumer.countdic.keys())[0]] and WSConsumer.countdic[list(WSConsumer.countdic.keys())[0]] > 1:
                    lst.append(list(WSConsumer.countdic.keys())[0])
                    l, p = disease(lst[-1])
                    l.remove('untitled-ontology-19.'+p)
                    l = list(set(l))
                    self.send(json.dumps({'message': "Disease Name:", 'message1': list(WSConsumer.countdic.keys())[
                              0], 'message2': p, 'message3': l, 'message4': "Want to move forward?"}))
                    WSConsumer.countdic.pop(
                        list(WSConsumer.countdic.keys())[0])
                    # print("Disease Name:",key)
                    # print("Do you wish to know if these diseases have any other symptoms??")
                    # ans=input("y/n")
                    # print("Symptoms: ",l)
                    ...
                    self.send(json.dumps(
                        {'message': "No match found for the combination of symptoms entered. Kindly reassess your symptoms and try again!"}))
                    self.close()
        else:
            self.close()
